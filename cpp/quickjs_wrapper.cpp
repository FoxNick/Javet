...
jsModuleLoaderFunc(JSContext *ctx, const char *module_name, void *opaque) {
    ...
    if (isBytecodeModule) {
        ...
        auto obj = JS_ReadObject(ctx, reinterpret_cast<const uint8_t*>(buffer), bufferLength, JS_READ_OBJ_BYTECODE);
...
jbyteArray QuickJSWrapper::compile(JNIEnv *env, jstring source, jstring file_name, jboolean isModule) const {
    ...
    auto buffer = JS_WriteObject(context, &bufferLength, compiled, JS_WRITE_OBJ_BYTECODE);
...
/*
jobjectArray QuickJSWrapper::getKeys(JNIEnv *env, jlong object_handle) const {
    JSValue this_obj = JS_MKPTR(JS_TAG_OBJECT, reinterpret_cast<void *>(object_handle));
    JSPropertyEnum *tab = NULL;
    uint32_t len = 0;
    int ret = JS_GetOwnPropertyNames(context, &tab, &len, this_obj, JS_GPN_STRING_MASK | JS_GPN_ENUM_ONLY);
    if (ret < 0) {
        return NULL;
    }
    jclass strClass = env->FindClass("java/lang/String");
    jobjectArray stringArray = env->NewObjectArray((int) len, strClass, nullptr);
    if (stringArray == NULL) {
        js_free_prop_enum(context, tab, len);
        return NULL;
    }
    for (int i = 0; i < len; ++i) {
        jstring key = env->NewStringUTF(JS_AtomToCString(context, tab[i].atom));
...
jobjectArray QuickJSWrapper::getKeys(JNIEnv *env, jlong value) const {
    ...
    if (stringClass == nullptr) {
        js_free_prop_enum(context, props, prop_count);
        return nullptr;
    }
...
void QuickJSWrapper::arrayAdd(JNIEnv *env, jobject thiz, jlong object_handle, jobject value) {
    ...
    int len = JS_VALUE_GET_INT(lenValue);
    JS_SetPropertyUint32(context, this_obj, len, toJSValue(env, thiz, value));
}
